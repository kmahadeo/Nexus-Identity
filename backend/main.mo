...
actor NexusIdentity {
  ...
  var userPasskeys = principalMap.empty<OrderedMap.Map<Text, PasskeyCredential>>();
  ...
  public query ({ caller }) func getDashboardData() : async UserDashboardData {
    // Compute security score dynamically from vault + recommendation counts
    let vaultCount = switch (principalMap.get(userVaults, caller)) {
      case (?vault) { textMap.size(vault) };
      case null { 0 };
    };
    let recCount = switch (principalMap.get(userRecommendations, caller)) {
      case (?recs) { textMap.size(recs) };
      case null { 0 };
    };
    // Base score 40, +5 per vault entry (max +40), -5 per open recommendation (min 0)
    let baseScore = 40;
    let vaultBonus = if (vaultCount > 8) { 40 } else { vaultCount * 5 };
    let recPenalty = if (recCount > 8) { 40 } else { recCount * 5 };
    let rawScore = baseScore + vaultBonus;
    let securityScore = if (rawScore > recPenalty) { rawScore - recPenalty } else { 0 };
  ...
  public shared ({ caller }) func addPasskeyCredential(credential : PasskeyCredential) : async () {
    let currentPasskeys = switch (principalMap.get(userPasskeys, caller)) {
      case (?passkeys) { passkeys };
      case null { textMap.empty<PasskeyCredential>() };
    };
    let updatedPasskeys = textMap.put(currentPasskeys, credential.credentialId, credential);
    userPasskeys := principalMap.put(userPasskeys, caller, updatedPasskeys);
  };

  public query ({ caller }) func getPasskeyCredentials() : async [PasskeyCredential] {
    switch (principalMap.get(userPasskeys, caller)) {
      case (?passkeys) { Iter.toArray(textMap.vals(passkeys)) };
      case null { [] };
    };
  };
  ...
  public shared ({ caller }) func deletePasskeyCredential(credentialId : Text) : async () {
    ...
    let updatedPasskeys = textMap.delete(currentPasskeys, credentialId);
    userPasskeys := principalMap.put(userPasskeys, caller, updatedPasskeys);
  };

  public query ({ caller }) func getPasskeyCount() : async Nat {
    switch (principalMap.get(userPasskeys, caller)) {
      case (?passkeys) { textMap.size(passkeys) };
      case null { 0 };
    };
  };

...
import { useGetVaultEntries } from '../hooks/useQueries';
...
export default function DeveloperPortal() {
  ...
  const [savedWebhooks, setSavedWebhooks] = useState<string[]>(() => {
    try {
      return JSON.parse(localStorage.getItem('nexus_webhooks') || '[]');
    } catch { return []; }
  });
  ...
  const handleGenerateApiKey = async () => {
    ...
    try {
      ...
      setApiKey(`nxs_${code}`);
      toast.success('API key generated via canister');
    } catch (error: any) {
      toast.error(error?.message || 'Failed to generate key (admin required)');
    } finally {
  ...
  const handleSaveWebhook = () => {
    ...
    const updated = [...savedWebhooks, url];
  ...
  const handleRemoveWebhook = (url: string) => {
    const updated = savedWebhooks.filter(w => w !== url);
    setSavedWebhooks(updated);
    localStorage.setItem('nexus_webhooks', JSON.stringify(updated));
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <Card className="border-border/40 glass-strong shadow-depth-md card-tactile gradient-primary">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-bold mb-1">{vaultEntries.length}</div>
            <div className="text-sm text-muted-foreground">Vault Operations</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/40 glass-strong shadow-depth-md card-tactile gradient-success">
          <CardContent className="p-6">
            ...
            <div className="text-sm text-muted-foreground">Canister Status</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/40 glass-strong shadow-depth-md card-tactile from-accent/15 to-accent/5">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-bold mb-1">{savedWebhooks.length}</div>
        ...
        <Card className="border-border/40 glass-strong shadow-depth-md card-tactile gradient-warning">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-bold mb-1">ICP</div>
              ...
              {savedWebhooks.length > 0 && (
                <div className="space-y-2">
                  ...
                  {savedWebhooks.map((url) => (
                    <div key={url} className="flex items-center gap-2 p-3 rounded-lg glass-effect border border-border/40">

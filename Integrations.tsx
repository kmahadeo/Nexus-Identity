...
export default function Integrations() {
  ...
  const handleConnect = async (integrationId: string) => {
    ...
    // Show backend requirement message
    toast.error('Backend OAuth2 integration required', {
      description: `Please implement /auth/${integrationId}/callback endpoint with secure token storage in Supabase. OAuth2 endpoint: ${integration.oauthEndpoint}`,
      duration: 5000,
    });

    // In production, this would redirect to OAuth2 authorization URL:
    // window.location.href = `/auth/${integrationId}/authorize`;
    
    // For demonstration, simulate the flow
    setIntegrations(prev =>
      prev.map(int =>
        int.id === integrationId ? { ...int, status: 'connecting' as IntegrationStatus } : int
      )
    );
    ...
    setTimeout(() => {
      setIntegrations(prev =>
        prev.map(int =>
          int.id === integrationId ? { ...int, status: 'error' as IntegrationStatus } : int
        )
      );
    }, 2000);
  };
